...
class PlayerPopupContentTest {
    ...
    @Test fun `quality toolbar keeps one accessible gear through portrait wide portrait layouts`() {
        for (fontScale in listOf(1f, 2f)) {
            ...
            val root = LayoutInflater.from(scaled).inflate(R.layout.player_layout, null) as ViewGroup
            val gear = root.findViewById<android.widget.ImageButton>(R.id.quality)
            gear.visibility = View.VISIBLE
            var clicks = 0
            gear.setOnClickListener { clicks++ }
            for ((width, height) in listOf(360 to 203, 800 to 360, 360 to 203)) {
                measure(root, width, height, View.MeasureSpec.EXACTLY)
                assertSame(gear, root.findViewById<View>(R.id.quality))
                assertNull("no width-dependent text replacement", root.findViewById<View>(R.id.qualityValue))
                assertEquals(View.VISIBLE, gear.visibility)
                assertNotNull(gear.drawable)
                assertEquals(scaled.getString(R.string.quality), gear.contentDescription)
                assertTrue("gear retains its touch target", gear.width >= 48 && gear.height >= 48)
                assertTrue(gear.performClick())
            }
